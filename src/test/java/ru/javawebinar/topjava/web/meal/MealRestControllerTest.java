...
class MealRestControllerTest extends AbstractControllerTest {
    ...
    void testGet() throws Exception {
        mockMvc.perform(get(REST_URL + MEAL1_ID).with(userAuth(USER)))
    ...
    void testDelete() throws Exception {
        mockMvc.perform(delete(REST_URL + MEAL1_ID).with(userAuth(USER)))
    ...
    void testUpdate() throws Exception {
        ...
        mockMvc.perform(put(REST_URL + MEAL1_ID).with(userAuth(USER)).contentType(MediaType.APPLICATION_JSON)
    ...
    void testCreate() throws Exception {
        ...
        ResultActions action = mockMvc.perform(post(REST_URL).with(userAuth(USER))
    ...
    void testGetAll() throws Exception {
        mockMvc.perform(get(REST_URL).with(userAuth(USER)))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void testFilter() throws Exception {
        mockMvc.perform(get(REST_URL + "filter").with(userAuth(USER))
    ...
    @Test
     void testFilterAll() throws Exception {
        mockMvc.perform(get(REST_URL + "filter?startDate=&endTime=").with(userAuth(USER)))
                .andExpect(status().isOk())
                .andDo(print())
